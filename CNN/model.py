...
def train_model(train_data):
  ...
  x = tf.compat.v1.placeholder(tf.float32, shape=[None, 2304], name='x')
  y_ = tf.compat.v1.placeholder(tf.float32, shape=[None, 7], name='y')
...
def predict(image=[[0.1] * 2304]):
  ...
  saver = tf.compat.v1.train.Saver()
  probs = tf.compat.v1.nn.softmax(y_conv)
  y_ = tf.argmax(probs)

  with tf.Session() as sess:
    # assert os.path.exists('/tmp/models/emotion_model')
    ckpt = tf.compat.v1.train.get_checkpoint_state('./models')
...
def valid_model(modelPath, validFile):
  ...
  saver = tf.compat.v1.train.Saver()
  ckpt = tf.compat.v1.train.get_checkpoint_state(modelPath)
